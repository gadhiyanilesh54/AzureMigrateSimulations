...
from azure_migrate_simulations.config import VCenterConfig
from azure_migrate_simulations.guest_discovery import GuestDiscoverer, Credential, DatabaseCredential, deep_probe_databases
from azure_migrate_simulations.workload_mapping import generate_workload_recommendations
from azure_migrate_simulations.enrichment import (
    ingest_telemetry,
    MonitoringTool,
    EnrichmentResult,
...
from azure_migrate_simulations.azure_pricing import AzureRetailPricing, set_default_client, resolve_paas_sku_tier
from azure_migrate_simulations.vulnerability_sla import analyse_vulnerability_sla
from azure_migrate_simulations.cloud_topology import generate_cloud_topology, get_waf_assessment
from azure_migrate_simulations.web.validation import require_fields, validate_int

# ---------------------------------------------------------------------------
# Resolve project root (two levels above src/azure_migrate_simulations/web/)
# ---------------------------------------------------------------------------
...
def _run_discovery(host: str, username: str, password: str,
    ...
    disc_logger = logging.getLogger("azure_migrate_simulations")
...
from azure_migrate_simulations.azure_mapping import VM_CATALOG as _VM_CATALOG_SOURCE
VM_SKU_CATALOG = {
...
from azure_migrate_simulations.workload_mapping import (
...
# ---------------------------------------------------------------------------
# Vulnerability & SLA API — OS lifecycle, software lifecycle, licensing
# ---------------------------------------------------------------------------

@app.route("/api/vulnerability-sla")
def api_vulnerability_sla():
    """Analyse discovered VMs and workloads against lifecycle databases.

    Returns OS end-of-life status, software end-of-support status,
    and Azure licensing guidance (AHUB, ESU, RI).
    """
    d = _load_data_or_404()
    result = analyse_vulnerability_sla(
        vms=d["vms"],
        recommendations=d["recommendations"],
        workload_data=_workload_data or None,
    )
    return jsonify(result)


# ---------------------------------------------------------------------------
# Cloud Topology Diagram API — CAF landing zones + WAF scoring
# ---------------------------------------------------------------------------

_ctd_cache: dict = {}


@app.route("/api/cloud-topology")
def api_cloud_topology():
    """Generate an Azure architecture topology from discovered data.

    Query params:
        region          – target Azure region (default: eastus)
        firewall        – true/false — enable Azure Firewall
        bastion         – true/false — enable Azure Bastion
        load_balancer   – true/false — enable Standard LB
        vpn_gateway     – true/false — enable VPN Gateway
    """
    global _ctd_cache
    d = _load_data_or_404()
    region = request.args.get("region", "eastus")
    optional_flags = {
        "azure_firewall": request.args.get("firewall", "false").lower() == "true",
        "bastion": request.args.get("bastion", "false").lower() == "true",
        "load_balancer": request.args.get("load_balancer", "false").lower() == "true",
        "vpn_gateway": request.args.get("vpn_gateway", "false").lower() == "true",
    }
    result = generate_cloud_topology(
        ...
        enrichment_data=_enrichment_data or None,
        perf_history=_perf_history or None,
        whatif_overrides=_whatif_overrides or None,
    )
    _ctd_cache = result
    return jsonify(result)


@app.route("/api/cloud-topology/waf/<resource_id>")
def api_cloud_topology_waf(resource_id: str):
    """Return detailed WAF assessment for a single resource."""
    if not _ctd_cache:
        return jsonify({"error": "No topology generated yet. Call /api/cloud-topology first."}), 404
    d = _load_data_or_404()
    result = get_waf_assessment(
        resource_id=resource_id,
        topology_data=_ctd_cache,
        vms=d["vms"],
        recommendations=d["recommendations"],
        enrichment_data=_enrichment_data or None,
        perf_history=_perf_history or None,
    ...
    if result is None:
        return jsonify({"error": f"Resource '{resource_id}' not found in the current topology."}), 404
    return jsonify(result)
...
def main():
    """Entry point for the ``dt-migrate-web`` console script."""
